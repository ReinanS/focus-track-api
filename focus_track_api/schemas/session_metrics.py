...
class SessionMetrics:
    ...
    def summary(self):
        avg_attention = sum(self.attention_scores) / len(self.attention_scores) if self.attention_scores else 0.0
        avg_fatigue = sum(self.fatigue_scores) / len(self.fatigue_scores) if self.fatigue_scores else 0.0
        avg_distraction = sum(self.distraction_scores) / len(self.distraction_scores) if self.distraction_scores else 0.0
        distraction_rate = (self.frames_distracted / self.frames_total) * 100 if self.frames_total > 0 else 0.0

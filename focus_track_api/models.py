...
class UserSettings:
    ...
    user_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey('users.id'), init=False, unique=True
    )
...
class DailySummary:
    ...
    sessions: Mapped[list['StudySession']] = relationship(
        back_populates='daily_summary', init=False
    )
    avg_fatigue: Mapped[float] = mapped_column(default=0.0)
    avg_distraction: Mapped[float] = mapped_column(default=0.0)
    focused_time: Mapped[int] = mapped_column(
        default=0
    )  # Tempo focado em milissegundos
    created_at: Mapped[datetime] = mapped_column(
        init=False, server_default=func.now()
    )
    updated_at: Mapped[datetime] = mapped_column(
...
class StudySession:
    ...
    critical_events: Mapped[Optional[str]] = mapped_column(Text, default=None)
    status: Mapped[str] = mapped_column(
        default='waiting'
    )  # waiting, active, paused, finished
    paused_at: Mapped[Optional[datetime]] = mapped_column(default=None)
    total_paused_time: Mapped[float] = mapped_column(
        default=0.0
